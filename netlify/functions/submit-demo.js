...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    console.log(`Processing Gmail SMTP submission from: ${name} (${email}) for company: ${company}`);
    ...
    const GMAIL_USER = process.env.GMAIL_USER; // e.g. amith@nexoraio.co.in
    const GMAIL_PASS = process.env.GMAIL_PASS; // 16-character App Password from Google Account

    if (!GMAIL_USER || !GMAIL_PASS) {
      console.error("GMAIL_USER or GMAIL_PASS environment variable is not defined.");
      return { 
        statusCode: 500, 
        body: JSON.stringify({ error: "Email service is temporarily misconfigured. Please check GMAIL_USER and GMAIL_PASS." }) 
      };
    }
    ...
    console.error("Error in Gmail SMTP submit-demo function:", error);
    return {
      statusCode: 500,
      headers: { 
        "Content-Type": "application/json",
